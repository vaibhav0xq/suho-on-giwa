...
export default function Home() {
  ...
  const requireSignedSession = useCallback(async () => {
    ...
    window.localStorage.removeItem(SESSION_STORAGE_KEY);
    setWalletPanelOpen(true);
    setMessage("Sign the wallet session to unlock protected actions.");
    try {
    ...
    <>
      ...
      <div className={"view view--intro" + (isLaunchingConsole ? " is-launching" : "")} id="intro">
        ...
        <main>
            ...
            <div className="wrap">
              <div className="hero-top" data-rise>
                <div>
                  ...
                  <p className="lede">Suho takes a sample of the recipient, measures it against tolerance, then holds value on a recallable route. A reading - not a guess - before you release.</p>
              ...
              <div className="infb" data-rise>
                <div className="c"><div className="h"><span className="dot" />What Suho protects against</div><p>Wrong-address transfers, address-poisoning look-alikes, and paying a recipient who is already flagged for scams. Suho reads the recipient first, so value never leaves on a guess.</p></div>
                <div className="c"><div className="h"><span className="dot" />What a reading is</div><p>A single measurement drawn from on-chain identity, a name reverse-match, and the risk registry - resolved into one verdict before you commit.</p></div>
              </div>
            </div>
          </section>
          ...
          <IntroSection id="s04" no="04" kicker="Settlement - preconfirmed to final" title="Watch the reading lock to final." body="Preconfirmed is an early Flashblocks signal - provisional, never final. Each stage latches as the value clears it.">
            <div className="bezel"><div className="bz-head"><span className="t">Settlement - live reading</span><span className="g" /><span className="live">LIVE</span></div><div className="steps">{timelineSteps.map((step, index) => <div key={step.key} className={"step " + (activeStage > index ? "done" : activeStage === index ? "cur" : "")}><span className="fl" /><span className="tk" /><div className="no">0{index + 1}</div><div className="nm">{step.label}</div><div className="ds">{step.detail}</div></div>)}</div><div className="settle__foot"><div><div className="msg">{message}</div><div className="sub">PRECONFIRMED IS NOT SETTLED - DO NOT TREAT AS FINAL.</div></div><a className="mono">{txHash ? shortAddress(txHash) + " \u2192" : "AWAITING REAL SEND"}</a></div></div>
          </IntroSection>
